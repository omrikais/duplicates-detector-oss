...
if [ -f "$REPO_ROOT/man/duplicates-detector.1" ]; then
    mkdir -p "$BUNDLE_OUTPUT/share/man/man1"
    cp "$REPO_ROOT/man/duplicates-detector.1" "$BUNDLE_OUTPUT/share/man/man1/"
    echo "  Copied man page"
else
    echo "  Warning: man/duplicates-detector.1 not found, skipping" >&2
fi

...
final class ScreenSnapshotTests: XCTestCase {
    ...
    func testMarketingScreenshot_imageComparison() throws {
        app.launchForUITesting(scenario: "marketing-image")
        XCTAssertTrue(app.waitForMainWindow())
        let startScan = app.buttons["Start Scan"]
        XCTAssertTrue(app.waitForElement(startScan, timeout: 10))
        startScan.click()
        ...
        XCTAssertTrue(app.waitForElement(app.buttons["Keep A"], timeout: 15))
        Thread.sleep(forTimeInterval: 2.0) // Allow thumbnail resolution

        captureMarketingScreenshot(named: "screenshot-comparison-photos")
    }

    func testMarketingScreenshot_videoComparison() throws {
        app.launchForUITesting(scenario: "marketing-video")
        XCTAssertTrue(app.waitForMainWindow())
        let startScan = app.buttons["Start Scan"]
        XCTAssertTrue(app.waitForElement(startScan, timeout: 10))
        startScan.click()
        ...
        captureMarketingScreenshot(named: "screenshot-comparison-videos")
    }

    func testMarketingScreenshot_insights() throws {
        app.launchForUITesting(scenario: "marketing-image")
        XCTAssertTrue(app.waitForMainWindow())
        let startScan = app.buttons["Start Scan"]
        XCTAssertTrue(app.waitForElement(startScan, timeout: 10))
        ...
        XCTAssertTrue(app.waitForElement(insightsButton, timeout: 5))
        insightsButton.click()
        Thread.sleep(forTimeInterval: 1.5) // Allow charts to render

        captureMarketingScreenshot(named: "screenshot-insights")
    }

    func testMarketingScreenshot_settings() throws {
        app.launchForUITesting(scenario: "marketing-image")
        XCTAssertTrue(app.waitForMainWindow())
        ...
        captureMarketingScreenshot(named: "screenshot-settings")
    }
    ...
    /// Capture a marketing screenshot and save it to the repo's `assets/` directory.
    ///
    /// Unlike `assertScreenshot`, this does not compare against a reference — it
    /// simply saves the PNG for use in marketing materials.
    private func captureMarketingScreenshot(named name: String) {
        let screenshot = app.windows.firstMatch.screenshot()
        let assetsDir = URL(fileURLWithPath: "/Volumes/TRANSCEND/repos/duplicates-detector/assets")
        try? FileManager.default.createDirectory(at: assetsDir, withIntermediateDirectories: true)
        let outputURL = assetsDir.appendingPathComponent("\(name).png")
        savePNG(screenshot.image, to: outputURL)

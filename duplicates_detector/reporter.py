...
from duplicates_detector.summary import _format_time
...
def _write_json_output(
    ...
    *,
    ...
    """Write JSON records with optional envelope wrapping and dry-run summary."""
    output = file if file is not None else sys.stdout
    if envelope is not None:
        payload: Any = {**envelope, key: records}
        if dry_run_summary is not None and dry_run_summary.deleted:
            payload["dry_run_summary"] = _build_dry_run_summary_dict(dry_run_summary, keep_strategy)
    elif dry_run_summary is not None and dry_run_summary.deleted:
        payload = {key: records, "dry_run_summary": _build_dry_run_summary_dict(dry_run_summary, keep_strategy)}
    else:
        payload = records
    _dump_json_pause_aware(payload, output, pause_waiter=pause_waiter)
    _pause_checkpoint(pause_waiter)
    output.write("\n")
...
def _write_shell_dry_run(output: TextIO, summary: DeletionSummary) -> None:
    """Append shell-comment dry-run summary."""
    output.write("\n# --- Dry Run Summary ---\n")
    output.write(f"# Files to delete: {len(summary.deleted)}\n")
    output.write(f"# Space recoverable: {format_size_human(summary.bytes_freed)}\n")
    output.write("# Run without --dry-run to execute deletions.\n")


def _write_markdown_dry_run(output: TextIO, summary: DeletionSummary) -> None:
    """Append markdown dry-run summary."""
    output.write("---\n\n")
...
def write_shell(
    ...
    *,
    ...
    if dry_run_summary is not None and dry_run_summary.deleted:
        _pause_checkpoint(pause_waiter)
        _write_shell_dry_run(output, dry_run_summary)
    ...
    for group in display_groups:
        ...
        for idx, member in enumerate(group.members, 1):
            ...
            table.add_row(
                ...
                format_size_human(member.file_size),
    ...
    *,
    ...
    for group in groups:
        ...
        for m in group.members:
            _pause_checkpoint(pause_waiter)
            thumb = thumbnails.get(m.path.resolve()) if thumbnails is not None else _THUMBNAIL_ABSENT
            f: dict[str, Any] = {
                "path": str(m.path),
                **_metadata_dict(m, thumbnail=thumb),
...
def write_markdown(
    ...
    *,
    ...
    if stats is not None and stats.total_time:
        header_parts.append(f"**Total time:** {_format_time(stats.total_time)}")
    ...
    # --- Dry-run summary ---
    if dry_run_summary is not None and dry_run_summary.deleted:
        _write_markdown_dry_run(output, dry_run_summary)
